...
const SC = {
  blue:   { border: "border-l-[3px] border-l-blue-500   dark:border-l-blue-400",   bg: "bg-blue-50/40   dark:bg-blue-950/20",   title: "text-blue-800   dark:text-blue-300",   icon: "text-blue-500   dark:text-blue-400"   },
  violet: { border: "border-l-[3px] border-l-violet-500 dark:border-l-violet-400", bg: "bg-violet-50/40 dark:bg-violet-950/20", title: "text-violet-800 dark:text-violet-300", icon: "text-violet-500 dark:text-violet-400" },
  emerald:{ border: "border-l-[3px] border-l-emerald-500 dark:border-l-emerald-400",bg:"bg-emerald-50/40 dark:bg-emerald-950/20",title:"text-emerald-800 dark:text-emerald-300",icon:"text-emerald-500 dark:text-emerald-400"},
  sky:    { border: "border-l-[3px] border-l-sky-500    dark:border-l-sky-400",    bg: "bg-sky-50/40    dark:bg-sky-950/20",    title: "text-sky-800    dark:text-sky-300",    icon: "text-sky-500    dark:text-sky-400"    },
  amber:  { border: "border-l-[3px] border-l-amber-500  dark:border-l-amber-400",  bg: "bg-amber-50/40  dark:bg-amber-950/20",  title: "text-amber-800  dark:text-amber-300",  icon: "text-amber-500  dark:text-amber-400"  },
  orange: { border: "border-l-[3px] border-l-orange-500 dark:border-l-orange-400", bg: "bg-orange-50/40 dark:bg-orange-950/20", title: "text-orange-800 dark:text-orange-300", icon: "text-orange-500 dark:text-orange-400" },
  cyan:   { border: "border-l-[3px] border-l-cyan-500   dark:border-l-cyan-400",   bg: "bg-cyan-50/40   dark:bg-cyan-950/20",   title: "text-cyan-800   dark:text-cyan-300",   icon: "text-cyan-500   dark:text-cyan-400"   },
  purple: { border: "border-l-[3px] border-l-purple-500 dark:border-l-purple-400", bg: "bg-purple-50/40 dark:bg-purple-950/20", title: "text-purple-800 dark:text-purple-300", icon: "text-purple-500 dark:text-purple-400" },
  slate:  { border: "border-l-[3px] border-l-slate-400  dark:border-l-slate-500",  bg: "bg-slate-50/40  dark:bg-slate-900/20",  title: "text-slate-700  dark:text-slate-300",  icon: "text-slate-500  dark:text-slate-400"  },
} as const;
...
function DetailField({ label, value }: { label: string; value: React.ReactNode }) {
  ...
  return (
    <div className="rounded-lg bg-background/70 px-3 py-2.5 ring-1 ring-border/50 dark:bg-muted/10 dark:ring-border/30">
      ...
      <p className={`text-sm font-medium leading-snug ${empty ? "italic text-muted-foreground/40" : "text-foreground"}`}>
        {empty ? "—" : value}
      </p>
    </div>
  );
...
function OccupancyBadge({ usageType }: { usageType: string }) {
  if (usageType === "self_occupied")
    return <Badge className="bg-emerald-600/15 text-emerald-700 hover:bg-emerald-600/15 dark:text-emerald-400">Self Occupied</Badge>;
  if (usageType === "rented")
    return <Badge className="bg-violet-600/15 text-violet-700 hover:bg-violet-600/15 dark:text-violet-400">Rented</Badge>;
    ...
    <div className="flex h-full flex-col overflow-hidden rounded-xl border border-border/60">
      <div className="relative aspect-video w-full overflow-hidden bg-muted">
        ...
        <div className="absolute left-3 top-3">
          <Badge
            variant={accuracyOk ? "default" : "destructive"}
...
function AreaMetricCard({
  label, sqft, sqm, color,
}: {
  label: string; sqft: number; sqm: string;
  color: "blue" | "violet" | "emerald";
}) {
  const styles = {
    blue:    "border-blue-200/60   bg-blue-50/60   text-blue-800   dark:border-blue-800/40   dark:bg-blue-950/30   dark:text-blue-300",
    violet:  "border-violet-200/60 bg-violet-50/60 text-violet-800 dark:border-violet-800/40 dark:bg-violet-950/30 dark:text-violet-300",
    emerald: "border-emerald-200/60 bg-emerald-50/60 text-emerald-800 dark:border-emerald-800/40 dark:bg-emerald-950/30 dark:text-emerald-300",
  };
  return (
...
function AreaSummaryGrid({ survey }: { survey: SurveyDetail }) {
  ...
  return (
    <div className="grid gap-3 sm:grid-cols-3">
      ...
      <AreaMetricCard label="Plinth Area"     sqft={areas.plinthSqft}  sqm={formatAreaSqMeter(areas.plinthSqft)}  color="violet"  />
      <AreaMetricCard label="Built-up Area"   sqft={areas.builtUpSqft} sqm={formatAreaSqMeter(areas.builtUpSqft)} color="emerald" />
    </div>
  );
}
  ...
  return (
    <div className="space-y-3">
        ...
        <Table>
          <TableHeader>
            <TableRow className="border-b border-border/60 bg-linear-to-r from-orange-50 to-amber-50/40 hover:from-orange-50 dark:from-orange-950/30 dark:to-amber-950/10 dark:hover:from-orange-950/30">
              <PropertyIdTableHead />
              <TableHead className="text-[10px] font-bold uppercase tracking-wider text-orange-700/70 dark:text-orange-400/70">Floor</TableHead>
              <TableHead className="text-[10px] font-bold uppercase tracking-wider text-orange-700/70 dark:text-orange-400/70">Usage</TableHead>
              <TableHead className="text-[10px] font-bold uppercase tracking-wider text-orange-700/70 dark:text-orange-400/70">Construction</TableHead>
              <TableHead className="text-right text-[10px] font-bold uppercase tracking-wider text-orange-700/70 dark:text-orange-400/70">Area (Sqft)</TableHead>
              <TableHead className="text-[10px] font-bold uppercase tracking-wider text-orange-700/70 dark:text-orange-400/70">Occupancy</TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            {floors.map((f, i) => (
              ...
              >
                ...
                <TableCell className="capitalize text-muted-foreground">
                  {labelFromOptions(masters?.usageTypes, f.usageType) || labelFromOptions(masters?.usageFactors, f.usageFactor)}
                </TableCell>
                <TableCell className="capitalize text-muted-foreground">
      ...
      <div className="flex items-center justify-between rounded-lg border border-orange-200/60 bg-orange-50/60 px-4 py-2.5 dark:border-orange-800/30 dark:bg-orange-950/20">
        <span className="text-xs font-bold uppercase tracking-wider text-orange-700/70 dark:text-orange-400/60">Total Built-up Area</span>
      ...
      <Table>
        <TableHeader>
          <TableRow className="border-b border-border/60 bg-muted/40 hover:bg-muted/40 dark:bg-muted/20">
            <PropertyIdTableHead />
            <TableHead className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground/70">When</TableHead>
            <TableHead className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground/70">Action</TableHead>
            <TableHead className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground/70">Actor</TableHead>
          </TableRow>
        </TableHeader>
        <TableBody>
          {audit.map((a, i) => (
            ...
            >
              <PropertyIdTableCell propertyId={propertyId} />
              <TableCell className="whitespace-nowrap text-xs text-muted-foreground">{fmtDate(a._creationTime)}</TableCell>
              <TableCell>
                <span className="rounded-md bg-muted/60 px-2 py-0.5 font-mono text-xs dark:bg-muted/30">{a.action}</span>
              </TableCell>
            ...
            >
              ...
              <div className="border-t border-border/40 bg-background/60 px-3 py-2 dark:bg-card/60">
                ...
                <p className="mt-0.5 text-[10px] text-muted-foreground">
                  {photo
                    ? `${fmtDate(photo.capturedAt)}${uploaderName ? ` · ${uploaderName}` : ""}`
                    : "Not uploaded"}
                </p>
              </div>
            </div>
    ...
    <div className="space-y-4">
      ...
      >
        <FieldGrid>
          ...
          <DetailField label="Mobile Number"         value={survey.mobileNo ? `+91 ${survey.mobileNo.replace(/^\+?91/, "")}` : undefined} />
            ...
            <Table>
              <TableHeader>
                <TableRow className="bg-violet-50/60 hover:bg-violet-50/60 dark:bg-violet-950/20 dark:hover:bg-violet-950/20">
                  <PropertyIdTableHead />
                  <TableHead className="text-[10px] font-bold uppercase tracking-wider text-violet-700/70 dark:text-violet-400/70">Name</TableHead>
                  <TableHead className="text-[10px] font-bold uppercase tracking-wider text-violet-700/70 dark:text-violet-400/70">Father / Husband</TableHead>
                  <TableHead className="text-[10px] font-bold uppercase tracking-wider text-violet-700/70 dark:text-violet-400/70">Mobile</TableHead>
                  <TableHead className="text-[10px] font-bold uppercase tracking-wider text-violet-700/70 dark:text-violet-400/70">Alt Mobile</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {owners.map((o, i) => (
      ...
      <div className="sticky bottom-0 z-10 -mx-1 overflow-hidden rounded-2xl border border-primary/20 bg-card/95 shadow-xl backdrop-blur-sm">
        ...
        <div className="flex flex-col gap-2 px-5 py-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-3">
            <div
              className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-linear-to-br from-indigo-500 to-blue-600 text-sm font-black text-white shadow-sm"
            >
              {progress}
            </div>
            <div>
          ...
          <div className="flex flex-col items-end gap-0.5">
            ...
            {survey.submittedAt && (
              <p className="text-[11px] font-semibold text-primary/80">
                Submitted {fmtDate(survey.submittedAt)}
              </p>
            )}
          </div>
        </div>
